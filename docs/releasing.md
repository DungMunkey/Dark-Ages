...
[`tools/package.ps1`](../tools/package.ps1), the same script you can run locally.
...
* **Version:** the base version from `src/Version.h` plus a suffix, e.g. `2.0.0-alpha.3-dev.47+a1b2c3d`
...
2. **Run the workflow.** On GitHub: *Actions* → *Release* → *Run workflow* (on `master`). It:
   ...
   * builds `master`, and
   * publishes a **pre-release** titled "Dark Ages 2.0.0-alpha.3" with tag `v2.0.0-alpha.3`.
...
5. **Start the next version.** On `dev`, bump `DA_VERSION_BASE` (e.g. to `2.0.0-alpha.4`) so development builds
...
mod in `Mods/`, `README.txt`, `LICENSE` and `THIRD-PARTY.txt`. Never: source code, object or `.pdb` files, Visual
Studio files, save games, `darkages.cfg`, or art-source files (`.xcf`, `.psd`, `.wav`, `.mid`, ...). The script
