Dark Ages: The Continents
Version {{VERSION}}
{{COPYRIGHT}}
==============================================================================
{{DEV_NOTE}}
HOW TO PLAY

  1. Extract the archive somewhere you can write to, for example:
       tar -xzf DarkAges-*-linux64.tar.gz
  2. Run ./Darkages from inside the DarkAges folder (or double-click it in your
     file manager).

  Requires a 64-bit (x86_64) Linux desktop with X11 or Wayland and PulseAudio,
  PipeWire or ALSA sound. SDL3, SDL3_ttf and SDL3_mixer are built into the
  program, so there is nothing to install.

  Controls: the arrow keys move; Enter or Space talks and selects; Esc opens
  and closes the menu; C casts a spell; Z shows your stats.

YOUR SAVE GAMES AND SETTINGS

  Save games are kept in the Saves folder next to the Darkages program, and your
  settings (window size, volume, which mod is selected) in darkages.cfg, which
  the game rewrites when you quit. Both are always kept beside the game, so keep
  the folder somewhere you can write to.

  This download comes with a darkages.cfg holding the default settings. To move
  to a newer version, extract it into a NEW folder and then copy your Saves
  folder and your darkages.cfg from the old folder into it. (Extracting on top
  of the old folder would replace your darkages.cfg with the default one.)

MODS

  Every folder inside Mods is a mod that can replace the game's graphics, maps
  and music. The game starts with the Project32 mod selected. Choose another
  from the Options menu; the change takes effect the next time you start the
  game.

LICENSE

  The game's code is released under the Apache License 2.0 (see LICENSE). It
  uses the SDL3, SDL3_ttf and SDL3_mixer libraries; their licenses are listed in
  THIRD-PARTY.txt.
